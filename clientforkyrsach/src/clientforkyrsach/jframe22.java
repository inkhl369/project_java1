...
private void tablica3()
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
         try
        {
            ...
            coos.writeObject("удалитьизтаблицыавтомобилей");
            coos.writeObject(ID);
        }
        catch(Exception e)
        {
...
save.setBackground(Color.white);
    ...
    private void redactframeWindowClosing(java.awt.event.WindowEvent evt) {
       delete.setEnabled(false);
        redaction.setEnabled(false);
    }
    ...
    private void addnameActionPerformed(java.awt.event.ActionEvent evt) {
...
public static boolean isDouble(String x) throws NumberFormatException
{
      ...
      try{
        if(addmodel.getText().length()>0 && addprice.getText().length()>0 && adddesc.getText().length()>0 && addother.getText().length()>0)
       {
           if(isDouble(addprice.getText())){
               ...
               addbutton.setBackground(Color.white);
                coos.writeObject("добавлениеавтомобиля");
   ...
   Point point ;
    ...
    private void zaprosActionPerformed(java.awt.event.ActionEvent evt) {
    Point point;
            point= getLocation();
    ...
    private void tovartableMouseClicked(java.awt.event.MouseEvent evt) {
